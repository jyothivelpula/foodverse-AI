"""Home page — hero, categories, featured menu, AI lounge, order status."""
...
def render() -> None:
    ...
    # Categories
    st.markdown('<div class="fv-section-title"><h3>Browse by category</h3></div>', unsafe_allow_html=True)
    ...
    # Featured menu
    st.markdown('<div class="fv-section-title"><h3>Featured Menu</h3></div>', unsafe_allow_html=True)
    ...
    # AI Lounge preview
    st.markdown('<div class="fv-section-title"><h3>🤖 AI Lounge</h3></div>', unsafe_allow_html=True)
    ...
    for col, persona in zip(persona_cols, HOME_PERSONAS):
        with col:
            ...
            if st.button("Chat", key=f"home_persona_{persona['display_name']}", use_container_width=True):
                set_persona(persona["key"])
                set_page("ai_lounge")
                st.rerun()

    # Order status
    st.markdown('<div class="fv-section-title"><h3>Order Status</h3></div>', unsafe_allow_html=True)
